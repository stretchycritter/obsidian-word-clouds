    ...
    try {
      ...
      await this.services.drawWordCloud({
        ...
        onExcludeInVault: async (word) => {
          ...
          new import_obsidian5.Notice(added ? `Excluded "${word}" from word clouds.` : `"${word}" is already excluded.`);
...
var RULES_WITHOUT_VALUE = /* @__PURE__ */ new Set(["exists", "not-exists"]);
var VaultWordCloudView = class extends import_obsidian6.ItemView {
  ...
  updateTagPickerOptions(selectEl, mode) {
  ...
  updateFolderPickerOptions(selectEl) {
  ...
  renderSelectedFolders(chipsEl, folderSelectEl, canvasEl) {
    chipsEl.empty();
  ...
  renderAppliedTagChips(chipsEl, tagSelectEl, mode, canvasEl) {
    chipsEl.empty();
  ...
  renderFrontmatterRules(rulesListEl, canvasEl) {
    rulesListEl.empty();
    ...
    this.filters.frontmatterRules.forEach((rule, index) => {
      ...
      this.registerDomEvent(operatorSelectEl, "change", () => {
        const nextOperator = operatorSelectEl.value;
  ...
  async persistFiltersAndRender(canvasEl, afterPersist) {
  ...
  buildFilterSummary() {
    const parts = [];
    parts.push(this.filters.scope.mode === "vault" ? "Scope: vault" : this.filters.scope.mode === "active-file" ? "Scope: active note" : `Scope: ${this.filters.scope.folderPaths.length || 0} folder(s)`);
...
var VaultWordCloudPlugin = class extends import_obsidian7.Plugin {
  ...
  getAvailableFolders() {
    return this.app.vault.getAllLoadedFiles().filter((file) => file instanceof import_obsidian7.TFolder).map((folder) => folder.path).sort((a, b) => a.localeCompare(b));
  }

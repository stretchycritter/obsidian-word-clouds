...
var EmbedWordCloudModal = class extends import_obsidian6.Modal {
  constructor(app, services, onInsert) {
    super(app);
    this.services = services;
  ...
  onOpen() {
    ...
    new import_obsidian6.Setting(this.modeWrapperEl).setName("Source").setDesc("Choose where this embedded cloud pulls words from.").addDropdown((dropdown) => {
    ...
    new import_obsidian6.Setting(contentEl).setName("Height").setDesc("Height of the embedded cloud in pixels.").addSlider((slider) => {
      slider.setLimits(180, 900, 10).setValue(this.state.height).setDynamicTooltip().onChange((value) => {
        this.state.height = value;
      });
    });
    new import_obsidian6.Setting(contentEl).setName("Enable interactions").setDesc("Allow zoom, pan, and click-to-search interactions.").addToggle((toggle) => {
    ...
    const cancelButton = new import_obsidian6.ButtonComponent(buttonRowEl).setButtonText("Cancel").onClick(() => {
      this.close();
    });
    cancelButton.buttonEl.type = "button";
    const insertButton = new import_obsidian6.ButtonComponent(buttonRowEl).setButtonText("Insert").setCta().onClick(() => {
      if (this.state.mode === "specific-file" && !this.state.filePath) {
        new import_obsidian6.Notice("Select an open markdown note before inserting.");
        return;
      }
  ...
  renderFileSetting() {
    ...
    new import_obsidian6.Setting(this.fileWrapperEl).setName("Specific note").setDesc("Use one open note as the source for this embedded cloud.").addDropdown((dropdown) => {
      const openFiles = this.services.getOpenMarkdownFiles();
  ...
  renderTagSetting() {
    ...
    new import_obsidian6.Setting(this.tagsWrapperEl).setName("Tags").setDesc(`Optional comma-separated tags. ${tagHint}`).addText((text) => {
  ...
  renderMatchSetting() {
    ...
    new import_obsidian6.Setting(this.matchWrapperEl).setName("Tag match mode").setDesc("When multiple tags are set, match any or all of them.").addDropdown((dropdown) => {
